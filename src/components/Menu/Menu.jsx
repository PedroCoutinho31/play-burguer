...
const SectionHeader = styled.div`text-align: center; margin-bottom: 48px;`;
...
const SectionTitle = styled.h2`
  ...
  color: #CC1414; text-shadow: 3px 3px 0px #000;
...
const Divider = styled.div`
  ...
  &::before, &::after {
    ...
    height: 2px; background: #CC1414; opacity: 0.5;
  }
  span { font-size: 18px; }
`;
...
const Tab = styled.button`
  ...
  border: 2px solid #FFD700; padding: 10px 16px;
  cursor: pointer; transition: all 0.1s; letter-spacing: 0.5px;
  &:hover {
    background: ${({ $active }) => ($active ? '#FFE566' : 'rgba(255,215,0,0.15)')};
    box-shadow: 0 0 12px rgba(255,215,0,0.4);
  }
  @media (max-width: 480px) { font-size: 7px; padding: 8px 12px; }
`;
...
// Card is now clickable (cursor: pointer already there)
const Card = styled.div`
  ...
  &:hover::before { background: #FFD700; }
`;
...
const CardDesc = styled.p`
  ...
  color: #666; line-height: 1.4; flex: 1;
`;
...
const Price = styled.span`
  ...
  color: #00FF41; text-shadow: 0 0 6px rgba(0,255,65,0.4);
`;
const AddBtn = styled.div`
  ...
  &:hover { background: #80FFB0; transform: scale(1.05); }
`;
const EmptyState = styled.div`
  text-align: center; padding: 60px 24px;
  font-family: 'Press Start 2P', cursive;
  color: #333; font-size: 10px; line-height: 2;
  div:first-child { font-size: 48px; margin-bottom: 16px; }
`;
const LoadingState = styled.div`
  text-align: center; padding: 80px 24px;
  div { font-family: 'Press Start 2P', cursive; color: #FFD700; font-size: 10px; letter-spacing: 3px; }
  &::after {
    content: ''; display: block; width: 60px; height: 4px;
    background: #FFD700; margin: 20px auto;
  ...
  return (
    <Section id="menu">
      <Container>
        ...
        <CategoryTabs>
          <Tab $active={activeCategory === 'todos'} onClick={() => setActiveCategory('todos')}>
            🕹️ TODOS
          </Tab>
          {categories.map(cat => (
        ...
        {loading ? (
          <LoadingState><div>CARREGANDO CARDÁPIO...</div></LoadingState>
        ) : filtered.length === 0 ? (
          <EmptyState>
            <div>🚫</div>
            <div>NENHUM ITEM</div>
          ...
          <Grid>
            {filtered.map((item, i) => (
              <Card key={item.id} $delay={`${(i % 8) * 0.05}s`} onClick={() => setSelectedItem(item)}>
                <CardImg>
                  {item.imageUrl
