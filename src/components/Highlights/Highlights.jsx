...
const titleFlicker = keyframes`
  0%, 18%, 22%, 25%, 53%, 57%, 100% {
    color: #FFD700; text-shadow: 3px 3px 0px #000, 0 0 10px rgba(255,215,0,0.4);
  }
  19%, 21%, 54%, 56% {
    color: #FFF; text-shadow: 3px 3px 0px #000, 0 0 30px #FFD700;
  }
`;
...
const Section = styled.section`
  ...
  background-image:
    repeating-linear-gradient(90deg, transparent, transparent 38px, rgba(255,215,0,0.03) 38px, rgba(255,215,0,0.03) 40px),
  ...
  &::before {
    ...
    background: repeating-linear-gradient(90deg, #FFD700 0px, #FFD700 20px, #CC1414 20px, #CC1414 40px);
  }
  &::after {
    content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 3px;
    background: repeating-linear-gradient(90deg, #CC1414 0px, #CC1414 20px, #FFD700 20px, #FFD700 40px);
  }
`;
const Container = styled.div`max-width: 1200px; margin: 0 auto;`;
const SectionHeader = styled.div`text-align: center; margin-bottom: 56px;`;
...
const SectionTitle = styled.h2`
  ...
  color: #FFD700; text-shadow: 3px 3px 0px #000;
...
const Divider = styled.div`
  ...
  &::before, &::after {
    content: ''; flex: 1; max-width: 120px; height: 3px;
    background: linear-gradient(to right, transparent, #FFD700);
  }
  &::after { background: linear-gradient(to left, transparent, #FFD700); }
  span { font-size: 20px; }
`;
...
const Card = styled.div`
  ...
  &:hover {
    border-color: #CC1414; transform: translateY(-6px);
    animation: ${cardHoverGlow} 2s ease-in-out infinite;
  }
  &::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px;
    background: #CC1414; transform: scaleX(0); transform-origin: left;
...
const CardBadge = styled.div`
  ...
  background: #CC1414; border: 2px solid #FF4040;
  padding: 4px 10px; font-family: 'Press Start 2P', cursive;
  font-size: 7px; color: #FFF; text-shadow: 1px 1px 0 #000; z-index: 1;
`;
...
const CardDesc = styled.p`
  ...
  color: #888; line-height: 1.5; margin-bottom: 20px;
`;
...
const AddBtn = styled.div`
  ...
  padding: 10px 14px; border: 2px solid #cc9d00;
      ...
      <Container>
        ...
        <Grid>
          {items.slice(0, 4).map((item, i) => (
            <Card key={item.id} $delay={`${i * 0.1}s`} onClick={() => setSelectedItem(item)}>
              <CardImage>
                {item.imageUrl
